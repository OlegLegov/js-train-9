...
function findElementGreaterThan(arr, num) {
  ...
  // Перевіряємо, чи є вхідний параметр масивом
  if (!Array.isArray(arr)) {
    return undefined;
  }
  // Знаходимо перший елемент, який більший за num
  let result = arr.find((element) => element > num);

  // Перевіряємо, чи знайдений елемент задовольняє умову
  if (result !== undefined) {
    return result;
  } else {
...
function findIndexByProperty(arr, name) {
  ...
  // Використовуємо метод findIndex для пошуку індексу об'єкта з властивістю 'name', яка співпадає з заданим ім'ям
...
function findLastEvenNumber(arr) {
  ...
  // Проходимо по масиву з кінця до початку
...
function getSubArrayAndConvertToString(arr, startIdx) {
  ...
  // Перевіряємо, чи є startIdx дійсним числом і чи не виходить він за межі масиву
  if (typeof startIdx !== "number" || startIdx < 0 || startIdx >= arr.length) {
    return "";
  }

  // Використовуємо метод slice для отримання підмасиву
  let subArray = arr.slice(startIdx);

  // Використовуємо метод join для конвертації підмасиву в рядок
...
function getArrayKeys(arr) {
  ...
  // Отримуємо ітератор ключів масиву за допомогою методу keys()
  let keysIterator = arr.keys();

  // Конвертуємо ітератор в масив за допомогою Array.from()
  let arrayKeys = Array.from(keysIterator);
...
function getPositiveArrayValues(arr) {
  ...
  // Отримуємо ітератор значень масиву за допомогою методу values()
  let valuesIterator = arr.values();

  // Конвертуємо ітератор в масив за допомогою Array.from()
  let valuesArray = Array.from(valuesIterator);

  // Використовуємо метод filter() для отримання лише додатніх значень
...
function removeAndDouble(arr, startIdx) {
  ...
  // Перевіряємо, чи startIdx є числом і чи він знаходиться в межах масиву
  if (typeof startIdx !== "number" || startIdx < 0 || startIdx >= arr.length) {
    return "Невірний індекс";
  }

  // Видаляємо елементи з масиву, починаючи з startIdx
  arr.splice(startIdx);

  // Подвоюємо залишені елементи за допомогою методу map()
...
function sumAndPrint(arr) {
  ...
  // Перевіряємо, чи вхідний параметр є масивом
  if (!Array.isArray(arr)) {
    console.error("Вхідний параметр має бути масивом");
    return;
  }

  // Використовуємо метод forEach() для виведення кожного елемента в консоль
...
function flattenSquareAndFilter(arr, threshold) {
  ...
  let flattenedArray = arr.flat(Infinity); // Використовуємо Infinity для повного вирівнювання

  // Підносимо кожен елемент до квадрату за допомогою методу map()
  let squaredArray = flattenedArray.map((value) => value * value);

  // Фільтруємо елементи за пороговим значенням за допомогою методу filter()
  let filteredSquaredArray = squaredArray.filter((value) => value > threshold);

  // Повертаємо масив піднятих до квадрату та профільтрованих елементів
...
function getLastElementsSumAndPrint(arr, n) {
  ...
  // Перевіряємо, чи n є числом і більше або дорівнює 0
  if (typeof n !== "number" || n < 0) {
    console.error("n має бути невід'ємним числом");
    return;
  }

  // Отримуємо останні n елементів масиву за допомогою методу slice()
  let lastElements = arr.slice(-n);

  // Виводимо кожен елемент у консоль за допомогою методу forEach()
  lastElements.forEach((element) => console.log(element));

  // Обчислюємо суму останніх n елементів за допомогою методу reduceRight()
  let sum = lastElements.reduceRight(
    (accumulator, currentValue) => accumulator + currentValue,
    0
  );

  // Повертаємо суму останніх n елементів
  return sum;
}
...
function wordLengths(arr) {
  ...
  if (!arr.every((item) => typeof item === "string")) {
    return "Всі елементи масиву повинні бути рядками";
  }

  // Використовуємо метод map() для створення нового масиву з довжинами слів
  let lengthsArray = arr.map((word) => word.length);

  // Повертаємо новий масив з довжинами слів
  return lengthsArray;
}
...
function insertElementAt(arr, index, element) {
  ...
  // Перевіряємо, чи index є числом і знаходиться в допустимих межах
  if (typeof index !== "number" || index < 0 || index > arr.length) {
    return "Індекс має бути невід'ємним числом і не перевищувати довжину масиву";
  }

  // Використовуємо метод splice() для вставки елемента на задану позицію
...
function flattenArray(arr) {
  ...
  // Використовуємо метод flat з параметром Infinity для "випрямлення" масиву на будь-яку глибину вкладеності
...
function findDuplicateElements(arr) {
  // Перевіряємо, чи вхідний параметр є масивом, якщо ні, повертаємо порожній масив
  if (!Array.isArray(arr)) {
    return [];
  }

  // Використовуємо filter(), щоб знайти дубльовані елементи
  let duplicates = arr.filter((item, index) => {
    // Елемент є дубльованим, якщо його перше входження в масив не збігається з поточним індексом
    return arr.indexOf(item) !== index;
  });

  // Видаляємо повтори з масиву дубльованих елементів, щоб кожен дубльований елемент з'явився лише один раз
  let uniqueDuplicates = duplicates.filter((item, index) => {
...
function capitalizeWords(arr) {
  ...
  // Використовуємо метод map() для перетворення кожного слова з першою великою літерою
...
function calculateTotalPrice(arr) {
  ...
  }, 0); // Початкове значення загальної суми - 0
